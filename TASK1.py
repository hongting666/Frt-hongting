...
def two_phase_sgld(distribution, params, q=0.95, gamma=1e-8,
    ...
    """
    Two‑phase SGLD:
      - Phase 1: large step to find approximate θ*
      - Phase 2: small step to finely sample around θ*
    Returns:
      thetas2 (second‑phase θ samples), xs2 (matching X samples)
    """
...
def estimate_var_std(thetas):
    """ Estimate VaRₛ𝗀𝗅𝗱 ± std from theta‑samples """
    m = thetas.mean()
    s = thetas.std(ddof=1)
    return m, s
...
def estimate_cvar_and_std(theta_samples, x_samples, q=0.95):
    ...
    then return mean±std for CVaRₛ𝗀𝗅𝗱.
    """
